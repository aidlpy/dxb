...
-(void)initUI{
    ...
    _appleBtn = [[UIButton alloc] initWithFrame:CGRectMake(35, bottom(_loginViewArray.lastObject)+48,SIZE.width-70,40)];
    _appleBtn.backgroundColor = Color_5ECAF5;
    [_appleBtn.layer setCornerRadius:2.0f];
    [_appleBtn setTitle:@"申请入驻" forState:UIControlStateNormal];
    [_appleBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [_appleBtn addTarget:self action:@selector(applyAciton:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)postRegister{
    ...
    [httpsManager postServerAPI:PostBRegister deliveryDic:dic successful:^(id responseObject) {
        ...
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            ...
            if ([[dic objectForKey:@"code"] integerValue] == 200)
            {
                ...
                if ([[[dic objectForKey:@"data"] objectForKey:@"error_code"] integerValue]== 0) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [SVHUD showSuccessWithDelay:@"注册成功！" time:0.8f blockAction:^{
                            BaseInfoViewController  *view = [BaseInfoViewController new];
                            view.usernameString =((LoginView *)(_loginViewArray[0])).textField.text;
                ...
                {
                    [SVHUD showErrorWithDelay:[[dic objectForKey:@"data"] objectForKey:@"msg"] time:0.8f];
