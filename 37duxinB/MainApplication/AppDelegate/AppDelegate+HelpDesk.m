...
-(void)initIMchat{
    ...
    options.apnsCertName = @"37duxincdevelopment";
...
-(void)registerIMChat{
   ...
   // [[EMClient sharedClient] registerWithUsername:FetchEMUsername password:FetchEMPassword];
    //登录使用user123 和 iostest 进行测试 密码均为123456 进去serviceModule 找到 ServiceViewController并找到 chatwithPaymentVC控制器。
    [[EMClient sharedClient] loginWithUsername:username password:emps completion:^(NSString *aUsername, EMError *aError) {
        if (!aError) {
            [[EMClient sharedClient].options setIsAutoLogin:YES];
            NSLog(@"登录成功-----");
        } else {
            NSLog(@"aError.code==>%ld",(long)aError.code);
