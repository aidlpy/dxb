//
//  ConsultingMainOrderFinishDetailViewController.m
//  37duxinB
//
...
@interface ConsultingMainOrderFinishDetailViewController ()<UITableViewDelegate,UITableViewDataSource>
{
...
@implementation ConsultingMainOrderFinishDetailViewController
...
-(void)fetchData{
    ...
    if (!_isMain) {
        ...
        if ([_orderStatus integerValue] == 0) {
             urlString = [NSString stringWithFormat:@"%@%@",FetchBconsultingWatingForPayDetail,_idString];
        }
    ...
    [httpManager getServerAPI:urlString deliveryDic:dic successful:^(id responseObject) {
        ...
        {
            if ([_orderStatus integerValue] == 0) {
                NSDictionary *resultDic = [[[responseObject objectForKey:@"data"] objectForKey:@"result"] objectForKey:@"result"];
...
-(void)initUI{
    ...
    if (!_isMain) {
        ...
        if ([_orderStatus integerValue] == 0) {
            _tableView.tableHeaderView = [self fetchTableviewHeader2:@"待咨询"];
        ...
        if ([_orderStatus integerValue] == 0) {
            _tableView.tableHeaderView = [self fetchTableviewHeader2:@"待支付"];
        }
        else{
            _tableView.tableHeaderView = [self fetchTableviewHeader:@""];
        }
...
-(UIView *)fetchTableviewHeader:(NSString *)string{
...
-(UIView *)fetchTableviewHeader2:(NSString *)titile{
